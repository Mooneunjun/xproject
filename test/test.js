...
function singerPart(member) {
  // 여기에 코드를 작성하세요
  let memberPartStart = lyrics.indexOf(`[${member}]`);
  let NextPartStart = lyrics.indexOf("[", memberPartStart + 1);

  // if (memberPartStart !== -1 && NextPartStart !== -1) {
  part = lyrics.slice(memberPartStart, NextPartStart);
  // } else if (memberPartStart !== -1) {
  //   part = lyrics.slice(memberPartStart);
  // }
